...
public struct APIServiceConstant {
    ...
    public enum RequestEndpoint {
        ...
        func getUrl(_ latitude: Double, _ longitude: Double) -> String {
            if let apiOpenWeatherMapAppId = Bundle.main.object(forInfoDictionaryKey: "API_KEY") as? String {
                let apiKey = String(format: apiOpenWeatherMapAppId.replacingOccurrences(of: "\"", with: ""))
                
                switch self {
                    case .current:
                        return "\(baseUrl)/data/2.5/weather?lat=\(latitude)&lon=\(longitude)&appid=\(apiKey)"
                    case .forecast:
                        return "\(baseUrl)/data/2.5/forecast?lat=\(latitude)&lon=\(longitude)&appid=\(apiKey)"
                }
            }
            return ""
        }
